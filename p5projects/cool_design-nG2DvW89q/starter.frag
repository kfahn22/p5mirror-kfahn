...
float tConnect( vec2 uv, float x1, float x2 ) {
  ...
  float s2 = sdBox(uv - vec2(x2, 0.), vec2(.04, 1.));
  float m2= S(.008, .0, s2);
  // float s3 = abs( sdCircle(uv - vec2(.57, .5), .5) ) - .04;
  //  float m3 = S(.008, .0, s3);
  // float s4 = abs( sdCircle(uv - vec2(.43, .5), .5) ) - .04;
 // float m4 = S(.008, .0, s4);
 float s3 =  abs(sdRoundedBox( gv - vec2(.57, .5), vec2(.5, .425), vec4(.35, .15, .25, .45)) ) -  .04;
    float m3 = S(.008, .0, s3);
  float s4 =  abs(sdRoundedBox( gv - vec2(.44, .5), vec2(.52, .55), vec4(.35, .15, .25, .45)) ) -  .04;
    float m4 = S(.008, .0, s4);
  return m1 + m2 + m3  + m4;
  //return m2;
}
...
void main()
{
  ...
  // col += s1 + s2 + s3 + s4 ;
   ...
   //col += (1. - b - str) * AQUA + b * RASPBERRY + str*PURPLE ;
    ...
    col += (1. - bcn) * AQUA + bcn * RASPBERRY ;//+  bcn * PURPLE;
