...
function setup() {
  ...
  let stack = [{ x: 0, y: 0, squareSize: rows }];

  while (stack.length > 0) {
    let current = stack.pop();
    let x = current.x;
    let y = current.y;
    let squareSize = current.squareSize;
    ...
    if (squareSize > resolution) {
      ...
      let newSize = floor(squareSize / 2) + 1;

      stack.push({ x: x, y: y, squareSize: newSize });
      stack.push({ x: x + (newSize - 1) * resolution, y: y, squareSize: newSize });
      stack.push({ x: x, y: y + (newSize - 1) * resolution, squareSize: newSize });
      stack.push({
