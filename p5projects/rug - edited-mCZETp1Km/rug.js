class PersianRug {
  constructor(x, y, shift, res, grid, size, rows, ncol, pg) {
    this.x = x;
    this.y = y;
    this.shift = shift;
    this.res = res;
  ...
  getCorners() {
    this.corners = [
      ...
      createVector(this.x + (this.size - 1) * this.res, this.y),
      createVector(this.x, this.y + (this.size - 1) * this.res),
      createVector(
        this.x + (this.size - 1) * this.res,
        this.y + (this.size - 1) * this.res
      ),
    ];
  }
