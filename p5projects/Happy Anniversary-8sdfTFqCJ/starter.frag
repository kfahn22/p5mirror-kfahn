...
float topI( vec2 uv ) {
  float s1 = sdBox( uv - vec2(-0.25, 0.0), vec2(0.25, 0.10));
  float m1 =  S(0.008, 0.0, s1);
  float s2 = sdBox( uv - vec2(0.0, 0.0), vec2(0.1, 0.25));
  float m2 =  S(0.008, 0.0, s2);
...
float quarterBox( vec2 uv ) {
 ...
 float rainbow( vec2 uv) {
   float s1 = sdCircle(uv - vec2(0.0, -0.5), 0.4);
   float m1 = S(.008, 0., s1);
   float s2 = sdCircle(uv - vec2(0.0, -0.5), 0.225);
   float m2 = S(.008, 0., s2);
   return m1 - m2;
   }
  
float twoRect( vec2 uv ) {
  float s1 = sdBox( uv - vec2(-0.3, 0.15), vec2(0.1, 0.35));
  float m1 = S(.008, 0., s1);
  float s2 = sdBox( uv - vec2(0.3, 0.15), vec2(0.1, 0.35));
  float m2 = S(.008, 0., s2);  
  return m1 + m2;
}
 ...
 float twoHalfCircles( vec2 uv) {
    float s1 = sdCircle(uv - vec2(-.25, -0.5), 0.25);
    float m1 = S(.008, 0., s1);
    float s2 = sdCircle(uv - vec2(0.25, -.5), 0.25);
    float m2 = S(.008, 0., s2);
    return m1 + m2;
   }

float triangle( vec2 uv ) {
  float s = sdBox( Rot(PI*1./4.) * uv - vec2(0.35, 0.35), vec2(0.35, 0.35));
  float m = S(.008, 0., s);
  return m;
}
...
vec3 chooseShape( float shapechoice, vec2 uv, vec3 col1, vec3 col2 ) {
  ...
  col += (1. - m3) * col2 + m3 * col1;
     ...
     //quarter circle
    float bc2 = twoHalfCircles(uv );
    col += (1. - bc2) * col1 + bc2 * col2;
  }
  else if (shapechoice == 5.0) {
    float qc = triangle(uv );
