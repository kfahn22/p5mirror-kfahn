// L-system pentaplexity rule from Paul Bourke
...
function setup() {
  ...
  palette.alpha = 255;
  fractal = lsystem.pentaplexity;
  setRule(fractal);

  // Set shape size as a fraction of length
  strokeWeight(2);
  fill(random(palette));
  selectedShape = new Craniod(0, 0, length * shapeScale, 1, 1, 1, radians(-41));
  selectedShape.addPoints();
  push();
  translate(width * 0.275, height * 0.225);
...
function addPalettes() {
  ...
  // Set default palette
  paletteDropdown.selected("fushia");
...
function selectPalette() {
  ...
  switch (currentPalette) {
    case "fushia":
      url = "https://supercolorpalette.com/?scp=G0-hsl-FF1FAD-FF1FBF-FF1FD2-FF1FE5-FF1FF8-F41FFF-E11FFF-CE1FFF";
      break;
    case "blue":
      url =
