...
// I have not incorporated this bit of code, but it is the next step to improveing efficiency
...
vec3 phongIllumination(vec3 k_a, vec3 k_d, vec3 k_s, float alpha, vec3 p, vec3 eye) {
    ...
    vec3 light1Pos = vec3(4.0 * sin(iFrame),
                          2.0,
                          4.0  * cos(iFrame));
...
void main() {
 ...
 //  add a target for the camera
  ...
  // The closest point on the surface to the eyepoint along the view ray
    ...
    vec3 K_a = vec3(0.1, 1., .5); // Ambient Color
    vec3 K_d = vec3(0.4, 0.4, 0.4);  //Diffuse color
    vec3 K_s = vec3(0.9, 0.9, 0.9);  // Specular color
    float shininess = 5.0;
