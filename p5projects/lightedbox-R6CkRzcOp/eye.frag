...
void main()
{
   ...
   if ( r < 0.8 )
   {
     ...
     float f = fbm( 4.0 * p ); // Adjust parameter here to make variation more pronounced
     ...
     f = smoothstep( 0.3, 1., fbm1( vec2( 6.*r, 20. *a) ) );
     col = mix( col, vec3(1.0), f);
     
      
     // Add black fibers
     f = smoothstep( 0.4, .9, fbm1( vec2( 10.*r, 15. *a) ) );
     col *= 1.0 - f;
     ...
     // Add reflection
     f = 1. - smoothstep( 0., .5, length( p - vec2(.25, .15) ) );
     col += vec3(1., .9, .8)*f*.8;
