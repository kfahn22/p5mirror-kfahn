...
void main()
{
    ...
    for (int u=0; u<3; u++){
        for (int v=0;v<3;v++){
            vec2 p =  (gl_FragCoord.xy+vec2(float(u),float(v))/3.0-0.5*u_resolution.xy)/s;
            // rotation and mirroring
    		float m = length(p);
    		float a = abs(mod(atan(p.y,p.x)+u_time,PI/3.0)-PI/6.0);
    		color.rgb += fractal(vec2(m*cos(a),m*sin(a)));
            //color.rgb = spectral_zucconi(m*cos(a));
        }
    }
