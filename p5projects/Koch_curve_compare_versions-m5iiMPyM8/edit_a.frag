...
float kochSDF( vec2 st, vec2 center, int N) {
    ...
    st.x = abs(st.x);
    // st.y += tan(5./6.*PI)*0.5;
    st.y += tan(1./6.*PI)*0.5; //adjustment to reorient Koch snowflake back in center
    ...
    st -= n*min(0.0, dot(st - vec2(0.0, 0.0), n))*2.; // uv.x*n.x + uv.y*n.y
    }
