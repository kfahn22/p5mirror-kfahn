...
function keyPressed() {
  if (key == "s") {
    ...
    saveGif("GIF/gear.gif", frames, options);
  }
}
...
function draw() {
  ...
  // If we add different colors to each directional light, we get stripes
   ...
   if (f <= 250) {
    ...
    g = 75;
    bl = 152;
  } else if (f > 750 && f <= 1000) {
    r = 181;
    g = 239;
    bl = 139;
     } else if (f > 1250 && f <= 1500) {
    r = 146;
    g = 213;
    bl = 230;
   } else if (f > 1500 && f <= 1750) {
    r = 211;
    g = 246;
    bl = 219;
  } else if (f > 1750 && f <= 2000) {
    r = 174;
    g = 236;
    bl = 239;
  }
  
  directionalLight(r, g, bl, 0, 0, -1);
  directionalLight(2,57, 74, 0, 0, 1);
  push();
