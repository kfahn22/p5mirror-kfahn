...
// mat2 m = mat2(r3, -3., 3., r3)*0.5; // 330 tree?
//vec2(3.,r3)*.5 - m*vec2(st.y, st.x);
...
vec2 ref (vec2 st, vec2 p, float angle) {
  ...
  return vec2(3.,r3)*.5 - m*vec2(st.y, abs(st.x));
}

// Reflects the UV based on a relfection line centered in the point p with a given angle
vec2 ref_org (vec2 uv, vec2 p, float angle) {
  vec2 dir = polarToCartesian (angle); // Direction of the reflection line
