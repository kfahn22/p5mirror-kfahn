...
void main() {
  ...
  float below = texture2D(currentBuff, pos + vec2(pix.x, 0.0))
  float bl = texture2D(currBuff, pos + vec2(pix.x, 0.0)).r;
  float br = texture2D(currBuff, pos - vec2(pix.x, 0.0)).r;

  // calculate the next state value
  float next = ((l + r)/2.0) - prev;
  next = next * damping;
