...
void main(void) {
    ...
    //sdf += flowerSDF(uv, 6);
    color.rgb += fill(sdf, 0.1)*vec3(0);
    
    //color.rgb += blendAdd(texture2D(u_texture, uv).rgb, ORANGE, sdf);
   
    //color.rgb = blendExclusion(texture2D(u_texture, uv).rgb, color.rgb, 0.5);
