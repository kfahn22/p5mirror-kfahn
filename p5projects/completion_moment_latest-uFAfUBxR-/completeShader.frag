...
void main( )
{
   ...
   // Adjust parameter here to make variation more pronounced
    ...
    vec3 ro = ta + vec3(1.5*sin(iTime),0.0,7.0*cos(iTime));  
  ...
  // Lens of camera
    vec3 rd = normalize( gv.x*uu + gv.y*vv + 20.0*ww*iTime );  
    ...
    for(float i=0.; i<14.; i+=3.) {    
    	vec4 layer = landscape(uv - vec2(0.0, -0.0004*iTime), dist, t+i, 3.0, amplitude, height, i, .01);
    ...
    col_alpha = mix(col_alpha, layers, layers.a);
