...
void main(  )
{
    ...
    //vec4 texColor0 = texture2D(text0, vec3(uv, 0.) );
    //vec3 texColor0 = vec3(texture2D(text0), .5); //texture2D depricated in OpenGL
    ...
    if(d<MAX_DIST) {
      ...
      // vec3 refOutside = texture(iChannel0, r).rgb;
        ...
        vec3 refrTex = mix(vec3(texColor0.rg, .5), rdOut, .2);
